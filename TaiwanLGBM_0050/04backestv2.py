...
import warnings
import matplotlib
matplotlib.use('Agg')  # 關掉 Tkinter backend，改用 Agg
...
# outcomes_new_features_* 裡 symbol 是 50 (int)
TARGET_SYMBOL = 50
...
if df.empty:
    raise ValueError(f"原始資料無 symbol={TARGET_SYMBOL} 資料，請確認 CSV 是否正確！")

test_start_date = pd.to_datetime('2019-01-09')
test_df = df[df['date'] >= test_start_date].copy()
if test_df.empty:
    raise ValueError(f"起始日期 {test_start_date} 後無 symbol={TARGET_SYMBOL} 資料！")
...
class SingleStockSystem:
    ...
    def try_entry(self, score, price, date):
        ...
        if self.buy_rule_type == 'original':  # 分級進場
            if score > 0.8:
                ratio = 1.0
            elif score > 0.7:
                ratio = 0.8

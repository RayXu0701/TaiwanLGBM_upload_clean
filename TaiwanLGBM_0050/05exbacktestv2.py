...
FEATURES_PATH = path_pc + 'features_list.txt'
...
# 仍然保留主資料的所有欄位（symbol, date, open, close ... 等都先保留）
with open(FEATURES_PATH, 'r', encoding='utf-8') as f:
    features = [line.strip() for line in f.readlines()]

# test_df 不要先 align_features, 只留原始特徵+日期等助查欄
...
model = lgb.Booster(model_file=path_pc + 'lightgbm_classify_model.txt')
...
for i in range(len(test_dates) - 1):
    ...
    signal_scores = {}
    for s in symbols:
        X_today = test_df[(test_df['date'] == date0) & (test_df['symbol'] == s)]
        if len(X_today)==0: continue
        X_pred = align_features(X_today, features) # <--- 強制補零對齊, 只對單日單股做，不要動 test_df 本體
        score = model.predict(X_pred.values.reshape(1, -1))[0][1]

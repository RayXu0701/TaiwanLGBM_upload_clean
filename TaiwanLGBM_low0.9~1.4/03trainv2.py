...
y_train = train_df['label_v2'].astype(int)
y_valid = valid_df['label_v2'].astype(int)
y_test  = test_df['label_v2'].astype(int)
...
def eval_pred(X, y, name):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
        xticklabels=['0:平','1:漲','2:跌'], yticklabels=['0:平','1:漲','2:跌'])
...
# ------ LightGBM 內建儲存方式 ------
model.save_model(path_pc + 'lightgbm_classify_model.txt')
with open(path_pc + 'features_list.txt', 'w', encoding='utf-8') as f:
    for col in features:
        f.write(col + '\n')
print("特徵欄位清單已存檔：", path_pc + "features_list.txt")

...
for symbol in valid_symbols:
    ...
    for i, idx in enumerate(symbol_df.index):
        if i + N < len(closes):
            ...
            df.loc[idx, 'future_max_return'] = future_max
            df.loc[idx, 'future_min_return'] = future_min
...
print("label分布:", df['label_v2'].value_counts(normalize=True).to_dict())

...
DATA_PATH = path_pc + 'outcomes_middle_features_step2_2025-10-23.csv'
FEATURE_PATH = path_pc + 'middlefeatures_recall_list.txt'
MODEL_PATH   = path_pc + 'lightgbm_middle_recall_model.txt'  # 回歸模型
...
class SmartTradingSystem:
    ...
    def buy_or_add(self, symbol, score, price, date):
        # 回歸分數門檻請依你模型分布測試調整
        if score > 0.03:      ratio = 1.0
        elif score > 0.025:   ratio = 0.8
        elif score > 0.02:    ratio = 0.6
        else:                 return False
    ...
    def sell(self, symbol, price, date, reason):
        ...
        self.trade_log.append({'date': date, 'symbol': symbol, 'side': 'sell', 'price': price, 'amount': position.amount,
                               'profit': profit, 'reason': reason, 'hold_days': position.days})
...
for i in range(len(test_dates) - 1):
    ...
    signal_scores = {}
    today_data = test_df[test_df['date'] == date0]
    for s in symbols:
        X_today = today_data[today_data['symbol'] == s][features].values
    ...
    if i % 100 == 0 and len(trading_system.portfolio_value_log) > 0:
        print(f'[Smart] {i}/{len(test_dates)} 資產：{trading_system.portfolio_value_log[-1]["portfolio_value"]:.0f}')

# ====== 主動策略績效報表 ======
if trading_system.trade_log:
...
for idx, date in enumerate(dates):
    if idx == 0 or (month_series[idx] != month_series[idx - 1] or year_series[idx] != year_series[idx - 1]):
        ...
        for s in symbols:
            ...
            if denom <= 0:
                continue
            buy_amt = min(X_PER_MONTH_PER_STOCK, remain, cash_dca / denom)
            if buy_amt > 0:
                units = buy_amt / open_price
...
plt.plot(portfolio_df['date'], portfolio_df['portfolio_value'], label='Active/Dynamic')
plt.plot(value_df_dca['date'], value_df_dca['portfolio_value'], label='DCA (1M total)')
plt.plot(value_df_eq['date'], value_df_eq['portfolio_value'], label='Equal Weight B&H')
plt.title('Portfolio Value: Active/Dynamic vs DCA vs Equal Weight B&H')
plt.xlabel('Date')

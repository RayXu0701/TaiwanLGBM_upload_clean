...
MODEL_PATH = path_pc + 'lightgbm_middle_recall_model.txt'
...
class SmartTradingSystem:
    ...
    def buy_or_add(self, symbol, score, price, date):
        # 回歸分數門檻請依你模型分布測試調整
        # 這裡設 >0.03, >0.025, >0.02(你模型mean=0.037)
...
for i in range(len(test_dates) - 1):
    ...
    for s in symbols:
        ...
        score = model.predict(X_today.reshape(1, -1))[0]  # 回歸模型是一個連續值
...
plt.title('Portfolio Value: Active vs DCA vs Equal B&H')
plt.xlabel('Date')
